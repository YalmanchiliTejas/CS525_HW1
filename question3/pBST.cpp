...
void start_t(op_t typ, int val, int thread_count){
  pthread_t t;
  op_v* ov = (op_v*)malloc(sizeof(op_v));
  ov->typ = typ;
  ov->val = val;
  pthread_mutex_lock(&act_l);
  while(thread_c == thread_count){
...
std::vector<op_v> generate_D(int m , unsigned seed){
    ...
    std::vector<int> Z1(vals.begin(), vals.begin() + z1_size);
    std::vector<int>temp = Z1;
    std::shuffle(temp.begin(), temp.end(), rng);
    std::vector<int> Z2(temp.begin(), temp.begin() + z2_size);
    std::uniform_int_distribution<int> dist_val(0, m - 1);
    std::vector<int> Z3(z3_size);
    for(int i = 0; i < z3_size; i++){
        Z3[i] = dist_val(rng);
    }

    std::uniform_real_distribution<double> dist_prop(0.0, 1.0);
    int total_size = z1_size + z2_size + z3_size;
    std::vector<op_v> D;
    D.reserve(total_size);
    for(int i = 0; i < z1_size; i++){
        D.push_back({op_t::OP_INSERT, Z1[i]});
    }
    for(int i = 0; i < z2_size; i++){
        D.push_back({op_t::OP_DELETE, Z2[i]});
    }
    for(int i = 0; i < z3_size; i++){
        op_t typ = dist_prop(rng) >= 0.5 ? op_t::OP_LOOKUP : op_t::OP_DELETE;
        D.push_back({typ, Z3[i]});
    }
...
int m = 3000000;
unsigned seed = 0;
std::vector<op_v> D = generate_D(m, seed);
...
for(int i = 1; i <= 128; i*= 2){
    ...
    for(size_t j = 0; j < D.size(); j++){
        op_v op = D[j];
        start_t(op.typ, op.val, i);
    }
    pthread_mutex_lock(&act_l);
    while(thread_c > 0) pthread_cond_wait(&act_cond, &act_l);
    pthread_mutex_unlock(&act_l);
    double end_time = seconds();
    printf("\nTotal time for %d operations with %d threads: %.2f seconds\n", (int)D.size(), i, end_time - start_time);
    free_tree(root);
    root = NULL;
}
}
